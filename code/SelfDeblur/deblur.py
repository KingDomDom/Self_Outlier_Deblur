...
parser.add_argument('--data_path', type=str, default="datasets/test/", help='path to blurry image')
...
for f in files_source:
    ...
    for step in tqdm(range(num_iter)):
        ...
        if (step+1) % opt.save_frequency == 0:
            ...
            out_x_np_8bit = (out_x_np_normalized * 255).astype(np.uint8)  # 转换为8位整数
            #从NumPy数组创建PIL图像对象
            img = Image.fromarray(out_x_np_8bit)
            img.save(save_path)
            ...
            out_k_np_8bit = (out_k_np * 255).astype(np.uint8)  # 转换为8位整数
            #从NumPy数组创建PIL图像对象
            img = Image.fromarray(out_k_np_8bit)
            img.save(save_path)
