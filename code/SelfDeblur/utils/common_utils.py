...
def adjust_kernel_center(kernel, h, w):
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    kernel = kernel.detach().cpu().numpy()
    X, Y = np.arange(h).reshape(1, 1, 1, -1), np.arange(w).reshape(1, 1, -1, 1)
    ...
    xshift = xc2 - xc1
    yshift = yc2 - yc1
    return torch.tensor(shift(kernel, [0, 0, yshift, xshift]), device=device)
