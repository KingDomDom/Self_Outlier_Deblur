...
class OID:
    ...
    def set_image(self, blurred_image):
        ...
        # hist, bins = np.histogram(blurred_image.flatten(), 256, density=True)
        # cdf = hist.cumsum()   #
        # cdf = cdf / cdf[-1]
        # # 使用累积分布函数的线性插值，计算新的像素值
        # blurred_image = np.interp(blurred_image.flatten(), bins[:-1], cdf).reshape(blurred_image.shape)
    ...
    def optimize(self, callback=None):
        for epoch in range(self.epoch):
            ...
            for step in range(self.epoch_K):
                ...
                self.K = conjugate_gradient(lambda x: grad_K(x) + b, b, torch.ones_like(self.K) / self.K.numel(), 21)
    ...
    def coarse_deblur(self, beta, lambd, kappa, beta_max, callback=None):
        for epoch in range(4):
            ...
            kernel_hat = psf_to_otf(self.K.clone()[0].flip(0).flip(1), (H, W)).unsqueeze(0)
            ...
            for step in range(1):
                ...
                grad_K = torch.func.jacrev(kernel_loss)
                b = -grad_K(torch.zeros_like(self.K))
                self.K = conjugate_gradient(lambda x: grad_K(x) + b, b, torch.ones_like(self.K) / self.K.numel(), 20)
    ...
    def estimate_latent(self, weight_grad_I=3e-4, pad=None, callback=None):
        ...
        return np.array(latent_image[:, pad:h + pad, pad:w + pad].cpu()).transpose(1, 2, 0)
