...
import pickle
from SelfDeblur.selfdeblur import SelfDeblur
...
if __name__ == '__main__':
    ...
    image = Image.open('./datasets/example_kernel4.png')
    blurred_image = (np.asarray(image) / 255).transpose(2, 0, 1)
    height, width = image.size

    kernels_size = [7, 11, 15, 21, 27] 
    
    kernel = np.loadtxt('./results/kernel.csv', delimiter=',')
    # latent_image = np.loadtxt('./image.csv', delimiter=',')#[::-1, ::-1]
    latent_image = np.array(Image.open('./results/latent.png'))/ 255
    ...
    image = image.crop((width/2 - 346, height/2 - 346, width/2 + 400, height/2 + 400))
    image.save('results/recover.bmp')
